...
import { TExchangeMarkprice } from "../common/types/exchange.type.js";
...
export class CrossCreateData {
  ...
  async setOrderbook(exchange: EExchange, symbol: string, asks: string[], bids: string[]) {
    const orderbookKey = RedisKeyMgr.MarketKey(exchange, symbol)
    const orderbook = {
      lastUpdateId: 0,
      E: Date.now(),
      T: 0,
  ...
  // 设置交易所的funding fee
  async setMarketData(exchange: EExchange, symbol: string, indexPrice: string, fundingFee: string) {
    const fundingFeeKey = `${exchange.toUpperCase()}:MARKET:0:FUTURE_U:TICKER:${symbol.toUpperCase()}`
  ...
  async setLINKData() {
    await this.setMarketData(EExchange.Aster, 'LINKUSDT', '101', '0.003')
    await this.setMarketData(EExchange.Backpack, 'LINK_USDC_PERP', '102', '0.0008')
  ...
  async setBtCData() {
    await this.setMarketData(EExchange.Aster, 'BTCUSDT', '110000', '0.000057')
    await this.setMarketData(EExchange.Backpack, 'BTC_USDC_PERP', '110001', '0.000058')
