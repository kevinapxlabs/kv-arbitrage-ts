export class TSMap<K, V> {
  ...
  /**
   * Get a specific item from the map by keys index.
   * 
   * @param {number} index
   * @returns {V}
   * 
   * @memberOf TSMap
   */
   private getByInddex(index: number): V {
   ...
   * @memberOf TSMap
   */
  public deepGet(path: K[]): V | null {
      if (!path || !path.length) return null;

      const recursiveGet = (obj: any, path: K[]): any => {
  ...
  public forEach(callbackfn: (value: V, key?: K, index?: number) => void): void {
      this._keys.forEach((v, i) => {
          callbackfn(this.getByInddex(i), v, i);
      });
  }
  ...
  public map(callbackfn: (value: V, key?: K, index?: number) => any): any[] {
      return this.keys().map((itemKey, i) => {
          return callbackfn(this.getByInddex(i), itemKey, i);
      });
  }


  /**
   * Create New Map based on a conditional function passed to filter.
  ...
  public filter(callbackfn: (value: V, key?: K, index?: number) => Boolean): TSMap<K, V> {
      let t = this;
      const newMap = new TSMap<K, V>()
      t.keys().forEach((v, i) => {
        const val = t.getByInddex(i)
        if (callbackfn(val, v, i)) {
          newMap.set(v, val)
        }
      })
      return newMap;
